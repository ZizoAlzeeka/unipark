...
@section('page-title', 'Parking Map')
@section('page-subtitle', 'View real-time availability and reserve your spot')
...
<div>
    ...
    <div class="grid grid-4 mb-24">
        ...
        @endphp
        <div class="zone-card">
            <div class="zone-card-header" style="{{ ['ZA'=>'background:var(--grad-primary)','ZB'=>'background:var(--grad-secondary)','ZC'=>'background:var(--grad-success)','ZD'=>'background:var(--grad-accent)'][$zone->code] ?? 'background:var(--grad-primary)' }}">
                ...
                <div style="color:#fff;">
                    ...
                    <div style="font-size:18px;font-weight:800;">{{ $zone->name }}</div>
                    <div style="font-size:12px;opacity:0.8;margin-top:2px;"><i class="fas fa-map-marker-alt"></i> {{ $zone->location ?? 'Campus' }}</div>
                </div>
            </div>
            <div class="zone-card-body">
                <div class="zone-availability">
                    ...
                    <div class="zone-stat">
                        <div class="zone-stat-value" style="color:var(--warning);">{{ $zone->spots->where('status','reserved')->count() }}</div>
                        <div class="zone-stat-label">Reserved</div>
                    </div>
                    <div class="zone-stat">
                        <div class="zone-stat-value" style="color:var(--danger);">{{ $zone->spots->where('status','occupied')->count() }}</div>
                        <div class="zone-stat-label">Occupied</div>
                    </div>
                </div>
                <div class="progress mt-12">
                    <div class="progress-bar {{ $rate > 50 ? 'success' : 'warning' }}" style="width:{{ $rate }}%;"></div>
                </div>
                <div style="font-size:12px;color:var(--text-muted);margin-top:6px;text-align:center;">{{ $rate }}% Available</div>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="card mb-24">
        ...
        <div id="gridView">
            @foreach($zones as $zone)
            <div class="zone-section" data-zone="{{ $zone->id }}">
                <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px 8px;">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <div style="width:38px;height:38px;background:{{ ['ZA'=>'var(--grad-primary)','ZB'=>'var(--grad-secondary)','ZC'=>'var(--grad-success)','ZD'=>'var(--grad-accent)'][$zone->code] ?? 'var(--grad-primary)' }};border-radius:var(--radius-sm);display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;font-weight:900;">{{ $zone->code }}</div>
                        <div>
                            <div style="font-size:15px;font-weight:700;">{{ $zone->name }}</div>
                            <div style="font-size:12px;color:var(--text-muted);">{{ $zone->spots->count() }} spots total</div>
                        </div>
                    </div>
                </div>

                <div class="parking-grid">
                    @foreach($zone->spots as $spot)
                    <div class="parking-spot {{ $spot->status }}" data-spot-id="{{ $spot->id }}" data-status="{{ $spot->status }}" data-zone="{{ $zone->id }}"
                         onclick="selectSpot({{ $spot->id }}, '{{ $spot->status }}', '{{ $zone->code }}-{{ $spot->spot_number }}')"
                         title="{{ $zone->code }}-{{ $spot->spot_number }} ({{ ucfirst($spot->type) }}) — {{ ucfirst($spot->status) }}">
                        <i class="spot-type-icon fas {{ ['standard'=>'fa-car','staff'=>'fa-briefcase','disabled'=>'fa-wheelchair','vip'=>'fa-star'][$spot->type] ?? 'fa-car' }}"></i>
                        <span class="spot-number">{{ $zone->code }}-{{ $spot->spot_number }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
...
<script async defer
    src="https://maps.googleapis.com/maps/api/js?key={{ config('services.google_maps.key') }}&libraries=places&callback=initMap&language=en">
</script>
@endpush
@endsection
