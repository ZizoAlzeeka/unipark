        ...
        <div>
            <div class="card mb-20">
                <div style="text-align:center;padding:20px 0 8px;">
                    ...
                    <h2 style="font-size:22px;font-weight:800;margin-top:16px;">{{ auth()->user()->name }}</h2>
                    <p style="color:var(--text-muted);font-size:14px;">{{ auth()->user()->email }}</p>

                    <div style="display:inline-flex;align-items:center;gap:6px;margin-top:8px;padding:6px 16px;border-radius:var(--radius-full);{{ auth()->user()->role === 'admin' ? 'background:rgba(239,71,111,0.1);color:var(--danger)' : (auth()->user()->role === 'staff' ? 'background:rgba(0,180,216,0.1);color:var(--secondary)' : 'background:rgba(108,99,255,0.1);color:var(--primary)') }};">
                        <i class="fas {{ auth()->user()->role === 'admin' ? 'fa-shield-alt' : (auth()->user()->role === 'staff' ? 'fa-chalkboard-teacher' : 'fa-user-graduate') }}"></i>
                        <span style="font-size:13px;font-weight:700;">{{ ucfirst(auth()->user()->role) }}</span>
                    </div>
                </div>
                ...
                <div style="margin-top:24px;padding-top:24px;border-top:1px solid var(--border-color);">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                        <div style="text-align:center;padding:16px;background:rgba(108,99,255,0.04);border-radius:var(--radius-md);">
                            <div style="font-size:24px;font-weight:800;color:var(--primary);">{{ $reservationStats['total'] }}</div>
                        ...
                        <div style="text-align:center;padding:16px;background:rgba(6,214,160,0.04);border-radius:var(--radius-md);">
                            <div style="font-size:24px;font-weight:800;color:var(--success);">{{ $reservationStats['completed'] }}</div>
                ...
                <div style="margin-top:16px;display:flex;flex-direction:column;gap:10px;">
                    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;border:1px solid var(--border-color);border-radius:var(--radius-md);">
                        ...
                        <span style="font-size:13px;font-weight:700;">{{ auth()->user()->university_id ?? '—' }}</span>
                    </div>
                    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;border:1px solid var(--border-color);border-radius:var(--radius-md);">
                        <span style="font-size:13px;color:var(--text-muted);">Phone</span>
                        <span style="font-size:13px;font-weight:700;">{{ auth()->user()->phone ?? '—' }}</span>
                    </div>
                    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;border:1px solid var(--border-color);border-radius:var(--radius-md);">
                        <span style="font-size:13px;color:var(--text-muted);">Vehicle Plate</span>
                        <span style="font-size:13px;font-weight:700;">{{ auth()->user()->vehicle_plate ?? '—' }}</span>
                    </div>
                    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;border:1px solid var(--border-color);border-radius:var(--radius-md);">
                        <span style="font-size:13px;color:var(--text-muted);">Member Since</span>
                        <span style="font-size:13px;font-weight:700;">{{ auth()->user()->created_at->format('M Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form action="{{ route('profile.update') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="email" class="form-control" value="{{ auth()->user()->email }}" readonly style="background:rgba(108,99,255,0.03);cursor:not-allowed;">
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="tel" name="phone" class="form-control" value="{{ old('phone', auth()->user()->phone) }}" placeholder="+966-17-000-0000">
                        </div>
                    </div>
                    ...
                    <div style="border-top:1px solid var(--border-color);margin:20px 0;padding-top:20px;">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                            <div class="form-group" style="margin-bottom:0;">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="vehicle_plate" class="form-control" value="{{ old('vehicle_plate', auth()->user()->vehicle_plate) }}" placeholder="ABC 123" style="text-transform:uppercase;">
                                </div>
                            </div>
                            <div class="form-group" style="margin-bottom:0;">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="vehicle_model" class="form-control" value="{{ old('vehicle_model', auth()->user()->vehicle_model) }}" placeholder="Toyota Camry">
                                </div>
                            </div>
                        </div>
                        <div class="form-group mt-12" style="margin-bottom:0;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="vehicle_color" class="form-control" value="{{ old('vehicle_color', auth()->user()->vehicle_color) }}" placeholder="White, Black, Silver...">
                            </div>
                        </div>
                    </div>
