...
@section('page-title', 'Book a Parking Spot')
@section('page-subtitle', 'Select a spot and define your reservation time')
            ...
            <form action="{{ route('reservations.store') }}" method="POST" id="reservationForm">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" id="zoneSelect" onchange="loadZoneSpots(this.value)" required>
                        ...
                        <option value="{{ $zone->id }}">{{ $zone->name }} ({{ $zone->spots->count() }} spots available)</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" name="spot_id" id="spotIdInput" value="{{ $selectedSpot?->id }}">
                    <div id="spotSelector" style="border:2px solid var(--border-color);border-radius:var(--radius-md);padding:16px;min-height:80px;text-align:center;color:var(--text-muted);font-size:14px;">
                        ...
                        <div id="selectedSpotDisplay" style="display:flex;align-items:center;justify-content:space-between;background:rgba(108,99,255,0.06);border:2px solid rgba(108,99,255,0.2);border-radius:var(--radius-md);padding:14px 18px;">
                            <div>
                                <div style="font-size:24px;font-weight:900;background:var(--grad-primary);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">{{ $selectedSpot->zone->code }}-{{ $selectedSpot->spot_number }}</div>
                                <div style="font-size:13px;color:var(--text-muted);">{{ $selectedSpot->zone->name }} — {{ ucfirst($selectedSpot->type) }}</div>
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="datetime-local" name="end_time" class="form-control" id="endTime"
                                   ...
                                   value="{{ old('end_time', now()->addHours(3)->format('Y-m-d\TH:i')) }}"
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="vehicle_plate" class="form-control"
                               placeholder="{{ auth()->user()->vehicle_plate ?? 'e.g. ABC 123' }}"
                               value="{{ old('vehicle_plate', auth()->user()->vehicle_plate) }}"
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" class="form-control" rows="3" placeholder="Any special notes or instructions...">{{ old('notes') }}</textarea>
                </div>
            ...
            <div class="card mb-20" id="summaryCard" style="background:linear-gradient(135deg,rgba(108,99,255,0.03),rgba(79,172,254,0.03));border:1px solid rgba(108,99,255,0.1);">
                ...
                <div style="display:flex;flex-direction:column;gap:12px;">
                    ...
                    <div style="display:flex;justify-content:space-between;padding:12px;background:#fff;border-radius:var(--radius-md);border:1px solid var(--border-color);">
                        ...
                        <span style="font-size:13px;font-weight:700;">{{ auth()->user()->vehicle_plate ?? 'Not set' }}</span>
                    </div>
                </div>
...
<script>
    ...
    selectedSpotData = {
        id: {{ $selectedSpot->id }},
        name: '{{ $selectedSpot->zone->code }}-{{ $selectedSpot->spot_number }}',
        zone: '{{ $selectedSpot->zone->name }}'
    };
