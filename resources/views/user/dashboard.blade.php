...
<div>
    ...
    <div class="grid grid-2" style="gap:24px;">
        ...
        <div class="card animate-fade-up stagger-3">
            ...
            <div style="background:linear-gradient(135deg,rgba(6,214,160,0.04),rgba(67,233,123,0.04));border:1px solid rgba(6,214,160,0.15);border-radius:var(--radius-md);padding:16px;margin-bottom:12px;" class="reservation-card active-card">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                    <div>
                        <div class="reservation-spot">{{ $res->spot->zone->code }}-{{ $res->spot->spot_number }}</div>
                        ...
                        <div style="font-size:12px;color:var(--text-secondary);margin-top:6px;display:flex;align-items:center;gap:6px;">
                            ...
                            {{ $res->start_time->format('M d, H:i') }} — {{ $res->end_time->format('H:i') }}
                        </div>
                    </div>
                    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;">
                        <span class="badge badge-success">Active</span>
                        @if($res->remaining_time)
                        <span style="font-size:11px;font-weight:700;color:var(--success);">{{ $res->remaining_time }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:8px;margin-top:12px;padding-top:12px;border-top:1px solid rgba(6,214,160,0.1);">
                    <span style="font-size:12px;color:var(--text-muted);">
                        <i class="fas fa-car" style="margin-right:4px;"></i>{{ $res->vehicle_plate ?? 'No plate' }}
                    </span>
                    <span style="margin-left:auto;display:flex;gap:8px;">
                        <a href="{{ route('reservations.show', $res->id) }}" class="btn btn-ghost btn-sm" style="font-size:12px;padding:4px 12px;">Details</a>
                        @if($res->canBeCancelled())
                        <form action="{{ route('reservations.cancel', $res->id) }}" method="POST" onsubmit="return confirm('Cancel this reservation?')">
                            @csrf
            ...
            <div style="text-align:center;padding:40px;color:var(--text-muted);">
                ...
                <a href="{{ route('reservations.create') }}" class="btn btn-primary btn-sm mt-12">Book a Spot Now</a>
            </div>
            @endforelse

            @if($activeReservations->count() > 0)
            <a href="{{ route('reservations.index') }}" style="display:block;text-align:center;font-size:13px;font-weight:600;color:var(--primary);margin-top:8px;">View All Reservations →</a>
            @endif
        </div>
        ...
        <div class="card animate-fade-up stagger-4">
            <div class="card-header">
                ...
                <a href="{{ route('parking-map') }}" class="btn btn-secondary btn-sm">
            ...
            <div style="padding:16px;background:rgba(108,99,255,0.02);border:1px solid rgba(108,99,255,0.06);border-radius:var(--radius-md);margin-bottom:12px;transition:var(--transition);" onmouseover="this.style.background='rgba(108,99,255,0.05)'" onmouseout="this.style.background='rgba(108,99,255,0.02)'">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                    <div>
                        ...
                        <div style="font-size:12px;color:var(--text-muted);">{{ $total }} total spots</div>
                    </div>
                    <div style="display:flex;gap:12px;font-size:12px;">
                        <span style="color:var(--success);font-weight:700;">{{ $available }} free</span>
                        <span style="color:var(--warning);font-weight:700;">{{ $reserved }} reserved</span>
                        <span style="color:var(--danger);font-weight:700;">{{ $occupied }} occupied</span>
                    </div>
                </div>
                <div class="progress">
                ...
                <div style="display:flex;justify-content:space-between;margin-top:6px;font-size:11px;color:var(--text-muted);">
                    ...
                    <a href="{{ route('parking-map') }}" style="color:var(--primary);font-weight:600;">Book →</a>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="card mt-24 animate-fade-up" style="animation-delay:0.5s;">
        <div class="card-header">
            ...
            <a href="{{ route('notifications.index') }}" class="btn btn-ghost btn-sm">View All</a>
        </div>

        <div style="display:flex;flex-direction:column;gap:8px;">
            @foreach($recentNotifications as $notif)
            <div style="display:flex;align-items:flex-start;gap:14px;padding:14px;border-radius:var(--radius-md);{{ !$notif->is_read ? 'background:rgba(108,99,255,0.04);border:1px solid rgba(108,99,255,0.1);' : 'border:1px solid var(--border-color);' }};transition:var(--transition);" onmouseover="this.style.background='rgba(108,99,255,0.04)'" onmouseout="this.style.background='{{ !$notif->is_read ? 'rgba(108,99,255,0.04)' : 'transparent' }}'">
                <div class="notif-item-icon {{ $notif->type }}" style="width:38px;height:38px;border-radius:10px;">
                    <i class="fas fa-{{ $notif->type_icon }}"></i>
                </div>
                <div style="flex:1;min-width:0;">
                    <div style="font-size:14px;font-weight:700;color:var(--text-primary);">{{ $notif->title }}</div>
                    <div style="font-size:13px;color:var(--text-muted);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $notif->message }}</div>
                    <div style="font-size:11px;color:var(--text-muted);margin-top:5px;font-weight:500;">{{ $notif->created_at->diffForHumans() }}</div>
                </div>
                @if(!$notif->is_read)
