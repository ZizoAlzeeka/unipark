...
<head>
    ...
    <title>@yield('title', 'Dashboard') — UniPark</title>
...
<body>
    ...
    <aside class="sidebar sidebar-new-theme" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <div class="nav-item">
                <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard') : route('dashboard') }}" class="nav-link {{ request()->routeIs('dashboard') || request()->routeIs('admin.dashboard') ? 'active' : '' }}">
    ...
    <header class="topbar topbar-new-theme">
        ...
        <div class="topbar-actions">
            ...
            <!-- User Avatar -->
            <a href="{{ route('profile') }}" style="display:flex; align-items:center; gap:10px; text-decoration:none; padding:6px 14px; border-radius:var(--radius-full); background:rgba(108,99,255,0.06); transition:var(--transition);"
               onmouseover="this.style.background='rgba(108,99,255,0.12)'" onmouseout="this.style.background='rgba(108,99,255,0.06)'">
                ...
                <span style="font-size:13px; font-weight:700; color:var(--text-primary); display:none;" class="d-sm-block">{{ auth()->user()->name }}</span>
            </a>
        </div>
    </header>
